...
def etiqueta_columna_planilla(columna):
    ...
    if texto.lower().startswith("fecha"):
        return f"F{texto[5:].strip()}" if texto[5:].strip() else "F"
    return f"F{texto}"
    ...
    if request.method == "POST" and form.is_valid():
        ...
        with transaction.atomic():
            nuevo_equipo = Equipo.objects.create(
                ...
                foto_asistente_tecnico=equipo.foto_asistente_tecnico,
        ...
        try:
            ...
            for fila in range(8, ultima_fila_jugadores + 1):
                ...
                if jugador_existente_equipo and jugador_ya_piso_cancha(jugador_existente_equipo):
                    cedulas_importadas.add(cedula)
                    if fecha_nacimiento != jugador_existente_equipo.fecha_nacimiento:
                        omitidos += 1
                        errores.append(
                            f"Fila {fila}: no se modificó la fecha de nacimiento de {jugador_existente_equipo.nombres} "
                    ...
                    jugador_existente_equipo.save(update_fields=["dorsal", "nombres"])
                    actualizados += 1
                    continue
